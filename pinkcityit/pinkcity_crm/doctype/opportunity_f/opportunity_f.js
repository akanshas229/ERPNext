...
function calculate_total_amount(frm) {
	var base_opportunity_amount = frm.doc.opportunity_amount * frm.doc.conversion_rate;
	frm.set_value('base_opportunity_amount', base_opportunity_amount)
}
...
function createNPDOpp(frm, cdt, cdn) {
	...
	frappe.call({
		...
		args: {
			...
			'product_specification': frm.doc.product_specification,
			...
			'description' : child.attachment_detail
		},
		error: function (obj) {
			console.log("hi227");
			console.log(obj);
...
function fetch_npd_status(frm, cdt, cdn) {
	var child = locals[cdt][cdn];
	if (child.product_development) {
		frappe.db.get_list("Product Development", {
			fields: ["*"],
			filters: { name: child.product_development },
		}).then((obj) => {
