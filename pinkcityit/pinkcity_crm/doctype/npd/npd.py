...
def create_npd_from_opp() :
	...
	designeer = frappe.form_dict.get("designeer", "")
	# designier = frappe.form_dict.get("designier", "")
	...
	req_for = frappe.form_dict.get("req_for")
	...
	if npd:
		...
		if req_for == "CUST2D-3D-Costing":
			doc.designer_name = designeer
			# doc.db_set('designer_name', designer)
		elif req_for == "PC-2D-3D-Costing" or "CUST-2D-3D-Costing" or "CUST-2D-Costing-3D":
			doc.designer = designeer
		...
		if req_for:
			if req_for == "CUST2D-3D-Costing":
				doc.db_set('workflow_state', 'Pending for 3D Design')
